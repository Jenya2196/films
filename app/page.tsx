...
export function generateMetadata() {
  ...
  return {
    title: `Новинки фильмы`,
    description: 'Новинки фильмы',
  };
}
...
export default async function Home({ searchParams }: Props) {
  ...
  const data = await fetchFromApi<tMove>(MOVIE_API_ENDPOINTS.nowPlaying(currentPage),true,7200).catch((error) => {
    notFound();
  })
  
  return (
    <BlockCards title="Новинки" data={data} currentPage={currentPage} href="/"/>
  );
}
